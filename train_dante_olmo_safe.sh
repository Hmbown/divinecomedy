...
# SAFE MODE: Resource-limited training to prevent CPU crashes
#
# Safety features:
#   - Memory monitoring before each stage
#   - Cooldown periods between stages (30s)
...
CONFIG="./olmo_lora_config_safe.yaml"  # Safe config with batch_size=1
OUTPUT_DIR="./dante_olmo_full"
LOG_FILE="./training_dante_olmo_full.log"
PROGRESS_FILE="./training_progress.txt"

# Safety thresholds
MIN_FREE_MEMORY_GB=8          # Minimum free RAM to continue (GB)
COOLDOWN_SECONDS=30           # Pause between stages
...
echo "============================================================" | tee -a "$LOG_FILE"
echo "Mode: SAFE (resource-limited)" | tee -a "$LOG_FILE"
echo "Model: $MODEL" | tee -a "$LOG_FILE"
echo "Config: $CONFIG" | tee -a "$LOG_FILE"
echo "Batch size: 2 | Max seq: 2048 (same as working config)" | tee -a "$LOG_FILE"
...
wait_for_memory $MIN_FREE_MEMORY_GB
cooldown 60  # Extra cooldown before fusion
